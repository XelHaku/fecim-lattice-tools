...
func (a *Array) MVMWithNonIdealities(input []float64, opts *MVMOptions) (*MVMResult, error) {
	...
	var irAnalysis *IRDropAnalysis // Declare outside scope for use in sneak path logic
	...
	for i := 0; i < a.config.Rows; i++ {
		...
		for j := 0; j < len(input); j++ {
			...
			if opts.EnableIRDrop && i < len(effectiveVoltages) && j < len(effectiveVoltages[i]) {
				// Use the effective voltage from the unified solver
				// effectiveVoltages[i][j] is now (V_wl - V_bl) and can be negative
				effV := effectiveVoltages[i][j]

				// Standard MVM expects G * V_input.
				// The unified solver gives us the actual potential difference.
				// We need to be careful: is vIn applied to the row?
				// Yes, AnalyzeIRDrop uses input[] to set row voltages.
				// So effectiveVoltages already includes the scaling from input[j].
				// However, MVM loop iterates over input[j] again.
				// Actually, AnalyzeIRDrop takes `input` as argument.
				// The solver computes the steady state with those inputs.
				// So CellCurrents in irAnalysis IS the actual current.

				// We should use the currents directly from the solver if available
				if irAnalysis != nil && i < len(irAnalysis.CellCurrents) {
					// irAnalysis.CellCurrents[i][j] is the bidirectional current from the solver.
					// We add it directly to the sum.
					// Note: MVM expects positive sum for accumulation usually, but real physics allows negative.
					// For digital/neuromorphic output, we might rectify or keep signed.
					// Keeping signed is more accurate for "Expected vs Actual".
					sum += irAnalysis.CellCurrents[i][j]
				} else {
					// Fallback (shouldn't happen if EnableIRDrop is true)
					sum += G * vIn * effV // This would be double counting if effV is absolute
				}
			} else {
				// Ideal case: V_in is just the DAC output
				sum += G * vIn
			}
		}

		// Sneak path logic is now INTEGRATED into the IR drop solver (Unified Solver).
		// We no longer add a separate analytical sneak current if IR drop is enabled.
		// The `sum` above (from CellCurrents) already includes all leakage paths.

		if opts.EnableSneakPaths && !opts.EnableIRDrop {
			// Only use analytical model if IR drop is DISABLED (legacy/fast mode)
			// Compute sneak current for this row (analytical approximation)
			sneakCurrent := a.computeSneakCurrentForRow(i, input, opts)
			sum += sneakCurrent

			// Populate simplified analysis for visualization
			if result.SneakPathAnalysis == nil {
				result.SneakPathAnalysis = &SneakPathAnalysis{
					SneakCurrents: make([][]float64, a.config.Rows),
				}
				for r := range result.SneakPathAnalysis.SneakCurrents {
					result.SneakPathAnalysis.SneakCurrents[r] = make([]float64, a.config.Cols)
				}
			}
		} else if opts.EnableSneakPaths && opts.EnableIRDrop {
			// If both enabled, we used the unified solver.
			// We can extract "sneak" components for visualization if needed.
			if result.SneakPathAnalysis == nil && irAnalysis != nil {
				result.SneakPathAnalysis = &SneakPathAnalysis{
					SneakCurrents: make([][]float64, a.config.Rows),
					TotalSignal:   0,
					TotalSneak:    0,
				}
				// Populate sneak map based on reverse currents or deviation from ideal
				for r := 0; r < a.config.Rows; r++ {
					result.SneakPathAnalysis.SneakCurrents[r] = make([]float64, a.config.Cols)
					for c := 0; c < a.config.Cols; c++ {
						// Heuristic: Negative current or current flowing when ideal is 0 = Sneak
						actualI := irAnalysis.CellCurrents[r][c]
						idealV := 0.0
						if c < len(input) {
							idealV = input[c]
						}
						...
						isSneak := false
						if idealV > 0 && actualI < 0 {
							isSneak = true
						} else if idealV == 0 && math.Abs(actualI) > 1e-12 {
							isSneak = true
						}

						if isSneak {
							result.SneakPathAnalysis.SneakCurrents[r][c] = math.Abs(actualI)
							result.SneakPathAnalysis.TotalSneak += math.Abs(actualI)
						} else {
							result.SneakPathAnalysis.TotalSignal += math.Abs(actualI)
						}
					}
				}
				if result.SneakPathAnalysis.TotalSignal > 0 {
					result.SneakPathAnalysis.AvgSneakRatio = result.SneakPathAnalysis.TotalSneak / result.SneakPathAnalysis.TotalSignal
				}
			}
		}
	...
	if opts.EnableSneakPaths {
		...
		isolationFactor := 1.0
		if opts.Is2T1R() {
			isolationFactor = 0.0001
		} else if opts.Is1T1R() {
			isolationFactor = 0.001
		}
...
// AnalyzeSneakPathsWithIsolation analyzes sneak paths for a target cell with optional isolation factor.
func (a *Array) AnalyzeSneakPathsWithIsolation(targetRow, targetCol int, isolationFactor float64) *SneakPathAnalysis {
	// Create analyzer
	analyzer := NewSneakPathAnalyzer(a.config.Rows, a.config.Cols)

	// Copy conductances (apply variation if needed, here just base)
	for i := 0; i < a.config.Rows; i++ {
		for j := 0; j < a.config.Cols; j++ {
			analyzer.SetConductance(i, j, a.cells[i][j].Conductance)
		}
	}

	// Run analysis
	analyzer.AnalyzeTarget(targetRow, targetCol, 1.0)

	// Convert to SneakPathAnalysis struct
	analysis := &SneakPathAnalysis{
		SneakCurrents: analyzer.SneakCurrents,
		TotalSignal:   analyzer.GetStats(1.0).TargetCurrent,
		TotalSneak:    analyzer.GetStats(1.0).TotalSneakCurrent,
	}

	// Apply isolation factor (scaling down sneak currents)
	if isolationFactor < 1.0 {
		analysis.TotalSneak *= isolationFactor
		for i := 0; i < a.config.Rows; i++ {
			for j := 0; j < a.config.Cols; j++ {
				analysis.SneakCurrents[i][j] *= isolationFactor
			}
		}
	}

	if analysis.TotalSignal > 0 {
		analysis.AvgSneakRatio = analysis.TotalSneak / analysis.TotalSignal
	}
